...
// User specific  ------------------------------------------------
  ...
  // message Count ----------------------------------------
    ...
    // In case pictures are not in top 30 of most used words
    sentPicsCount[i] = !isNaN(Words['_<media']) ? Words['_<media'] : 0;
...
// chronological Graph
  ...
  formattedData = formatAll(messageCount, datesFormated);
  ...
  var cfg = {
      type: 'line',
      data: {
      labels: formattedData[0][0],
      datasets: [{
        label: contents[0].name,
        data: formattedData[0][1][0],
        type: 'line',
        fill: true,
        steppedLine: true,
        pointRadius: 0,
        ...
        borderWidth: 1,
        pointHoverRadius: 10,
        backgroundColor:"rgba(20, 168, 204, 0.4)",
        borderColor:"rgb(20, 168, 204)",
        pointBackgroundColor:"rgb(20, 168, 204)",
        pointBorderColor:"#fff",
        pointHoverBackgroundColor: "#fff",
        ...
        borderWidth: 1,
        pointHoverRadius: 10,
        backgroundColor:"rgba(255, 72, 64, 0.4)",
        borderColor:"rgb(255, 72, 64)",
        pointBackgroundColor:"rgb(255, 72, 64)",
        pointBorderColor:"#fff",
        pointHoverBackgroundColor:"#fff",
...
//
// add as html
var wordPDA = Math.round((wordsPerMessage[0][1]+wordsPerMessage[1][1])/(formattedData[0][0].length));
var messPDA = Math.round((messagesCount[0]+messagesCount[1])/(formattedData[0][0].length));
var div = document.createElement('div');
div.className = 'mb-0';
...
function createArray(contents) {
  ...
  for (var i = 0; i < contents.length; i++) {
    // search for DD/MM/YYYY, HH:MM - using RegEx and note index of first number
    testString = contents.substring(i, i + 19);
    
    var re = /(\d{2}\/){2}\d{4}\, \d{2}\:\d{2} \-/;
  ...
  for (var i = 0; i < lineArray.length; i++)  {
    if (lineArray[i].substring(19, lineArray[i].length).indexOf(":") < 0) {
...
function createStructs(lineArray) {
  ...
  for (i = 0; i < uniqueNames.length; i++) {
    ...
    for (j = 0; j < lineArray.length; j++) {
      if ( lineArray[j].substring(20, 20 + nameLength).match(name) ) {

        date[a] = lineArray[j].substring(0, 10);
        time[a] = lineArray[j].substring(12, 17);
        message[a] = lineArray[j].substring(22 + uniqueNames[i].length);
        a++;
      }
    }
    // console.log(message);
  ...
  // return
  // console.log("structArray" + JSON.stringify(structArray));
  return structArray;
}
...
function findNames(lineArray) {
  ...
  for (var i = 0; i< lineArray.length; i++){
    ...
    var secondIndex = lineArray[i].substring(20, lineArray[i].length - 1).indexOf(": ") + 20;
...
function formatDates(datesToFormat) {
  ...
  for (i = 0; i < datesToFormat.length; i ++) {
    // DD MM YY-> D MMM YY
    var date = datesToFormat[i].substring(6, 10) + " " + datesToFormat[i].substring(3, 5) + " " + datesToFormat[i].substring(0, 2);
...
function formatAll(data, datesF) {
  ...
  for(i = 0; i < data.length; i++) {
    ...
    // console.log("-------- ENTRY TO formatALL -----------");
    // console.log(i);
    ...
    var dates = getDates(new Date(datesToCheck[0].substring(6, 10), 
      parseInt(datesToCheck[0].substring(3, 5)) - 1,
      datesToCheck[0].substring(0, 2)), 
      new Date(datesToCheck[datesToCheck.length-1].substring(6,10),
      datesToCheck[datesToCheck.length-1].substring(3, 5) - 1,
      datesToCheck[datesToCheck.length-1].substring(0, 2)));
    ...
    for (k = 0; k < dates.length; k ++) {
      ...
      counts[i] = countsT;
      dates[k] = moment(dates[k], 'D MMM YY').format('YYYY-MM-DD')
    }
  }
...
function getMessagesPerDay(dates) {
  ...
  // check and reformat date to american standard MM.DD.YYYY
  var reformat = false;
  ...
  for (i = 0; i < dates.length; i++) {
    if (reformat) {
     // var d = new Date("20" + dates[i].substring(6,7) + "-" + dates[i].substring(3,5) + "-" + dates[i].substring(0,2));
      var d = new Date(dates[i].substring(6, 10) + "-" + dates[i].substring(3, 5) + "-" + dates[i].substring(0, 2))
    } else {
      // var d = new Date(("20" + dates[i].substring(6,7) + "-" + dates[i].substring(0,2) + "-" + dates[i].substring(3,5)));
      var d = new Date(dates[i].substring(6, 10) + "-" + dates[i].substring(0, 2) + "-" + dates[i].substring(3,5));
    }
    var dayNum = d.getDay();
